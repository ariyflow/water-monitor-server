...
- **认证**：**不需要登录**（设备自删，用用户名鉴权）。
- **说明**：删除指定设备及其全部传感器数据、报警记录与阈值设置（用户账号保留）。设备重置（KEY3 长按）时调用。
- **路径参数**：`serial`（设备序列号）。
- **请求体（JSON）**：`{"username": "..."}`（设备归属用户名，必须与设备所属用户匹配）。
- **成功响应**（`200`）：`{"message": "设备已删除"}`
- **错误**：
  - `400` `{"error": "Field 'username' is required"}`
  - `403` `{"error": "用户名与设备不匹配"}`
...
设备默认用固件内置宏作为报警阈值；设备上电后可从服务器拉取各设备的独立阈值（`GET /api/settings`），服务器端允许设备所属用户编辑（`PUT /api/settings`）。未配置任何阈值时按固件默认值返回（温度 0~50℃、流量 20、电导率 500、浊度 40、PH 4.0~10.0）。
...
- **成功响应**（`200`）：
  ...
  {"data": {"serial": "e7980eab5386", "temp_low_c": 0.0, "temp_high_c": 50.0,
            "flow_high_lpm": 20.0, "ec_high_us_cm": 500.0, "turb_high_ntu": 40.0,
